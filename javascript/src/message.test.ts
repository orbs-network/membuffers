...
test("TestMessageReadUint256", () => {
  const tests: any[] = [
    {
      ...
      scheme: [FieldTypes.TypeUint32, FieldTypes.TypeUint256],
      unions: [],
      fieldNum: 1,
      expected: BigInt("0x2030405060708090a0b0c0d0e0e0f0102030405060708090a0b0c0d0e0e0f"),
    },
    {
      buf: new Uint8Array([0x44, 0x00, 0x00, 0x00,
      ...
      scheme: [FieldTypes.TypeUint8, FieldTypes.TypeUint256],
      unions: [],
      fieldNum: 1,
      expected: BigInt("0x1002030405060708090a0b0c0d0e0e0f0102030405060708090a0b0c0d0e0eee"),
    },
    {
      buf: new Uint8Array([0x2,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x20,0x20,0x2,0x0,0x0,0x0]),
      scheme: [FieldTypes.TypeUint32, FieldTypes.TypeUint256, FieldTypes.TypeUint32],
      unions: [],
      fieldNum: 1,
      expected: 8224n,
    },
  ...
  for (const tt of tests) {
    const m = new InternalMessage(tt.buf, tt.buf.byteLength, tt.scheme, tt.unions);
    const s = m.getUint256(tt.fieldNum);
    // console.log(tt); // uncomment on failure to find out where
    expect(s).toBe(tt.expected);
  }
});
...
test("TestMessageReadBytes20", () => {
  const tests: any[] = [
    {
      buf: new Uint8Array([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]),
      scheme: [FieldTypes.TypeBytes20],
      unions: [],
      fieldNum: 0,
      expected: new Uint8Array([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]),
    },
    {
      buf: new Uint8Array([0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x9, 0xa, 0xb, 0xc, 0xd, 0xe, 0xe, 0xf, 0x10, 0x11, 0x12, 0x13]),
      scheme: [FieldTypes.TypeBytes20],
      unions: [],
      fieldNum: 0,
      expected: new Uint8Array([0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x9, 0xa, 0xb, 0xc, 0xd, 0xe, 0xe, 0xf, 0x10, 0x11, 0x12, 0x13]),
    },
    {
      buf: new Uint8Array([0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x9, 0xa, 0xb, 0xc, 0xd, 0xe, 0xe, 0xf, 0x10, 0x11, 0x12, 0x13,
        0xab, 0x2, 0x3, 0xab, 0x5, 0x6, 0xab, 0x8, 0x9, 0xa, 0xb, 0xc, 0xd, 0xe, 0xe, 0xf, 0x10, 0x11, 0x12, 0xff]),
      scheme: [FieldTypes.TypeBytes20, FieldTypes.TypeBytes20],
      unions: [],
      fieldNum: 1,
      expected: new Uint8Array([0xab, 0x2, 0x3, 0xab, 0x5, 0x6, 0xab, 0x8, 0x9, 0xa, 0xb, 0xc, 0xd, 0xe, 0xe, 0xf, 0x10, 0x11, 0x12, 0xff]),
    },
    {
      buf: new Uint8Array([0x01, 0x00, 0x00, 0x00,
        0xab, 0x2, 0x3, 0xab, 0x5, 0x6, 0xab, 0x8, 0x9, 0xa, 0xb, 0xc, 0xd, 0xe, 0xe, 0xf, 0x10, 0x11, 0x12, 0xff,
        0x88, 0x77, 0x66, 0x55]),
      scheme: [FieldTypes.TypeUint8, FieldTypes.TypeBytes20, FieldTypes.TypeUint32],
      unions: [],
      fieldNum: 1,
      expected: new Uint8Array([0xab, 0x2, 0x3, 0xab, 0x5, 0x6, 0xab, 0x8, 0x9, 0xa, 0xb, 0xc, 0xd, 0xe, 0xe, 0xf, 0x10, 0x11, 0x12, 0xff]),
    },
  ...
  for (const tt of tests) {
    const m = new InternalMessage(tt.buf, tt.buf.byteLength, tt.scheme, tt.unions);
    const s = m.getBytes20(tt.fieldNum);
    // console.log(tt); // uncomment on failure to find out where
    expect(s).toBeEqualToUint8Array(tt.expected);
  }
});
...
test("TestMessageMutateUint256", () => {
  const tests: any[] = [
    ...
    },
    {
      ...
      unions: [],
      fieldNum: 1,
      expected: new Uint8Array([0x44, 0x00, 0x00, 0x00,
        0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55,
        0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55]),
      err: false,
    },
  ...
  for (const tt of tests) {
    ...
    if (tt.err) {
      expect(() => {
        m.setUint256(tt.fieldNum, BigInt("0x555555555555555555555555555555555555555555555555555555555555555555"));
      }).toThrow();
    } else {
      expect(() => {
        m.setUint256(tt.fieldNum, BigInt("0x5555555555555555555555555555555555555555555555555555555555555555"));
      }).not.toThrow();
    }
    expect(tt.buf).toBeEqualToUint8Array(tt.expected);
  }
...
test("TestMessageMutateBytes20", () => {
  const tests: any[] = [
    {
      buf: new Uint8Array([0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x9, 0xa, 0xb, 0xc, 0xd, 0xe, 0xe, 0xf, 0x01, 0x55, 0x55, 0x01]),
      scheme: [FieldTypes.TypeBytes20],
      unions: [],
      fieldNum: 0,
      newField: new Uint8Array([0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55]),
      expected: new Uint8Array([0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55]),
      err: false,
    },
    {
      buf: new Uint8Array([0x40, 0x00, 0x00, 0x0,0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x9, 0xa, 0xb, 0xc, 0xd, 0xe, 0xe, 0xf, 0x10, 0x11, 0x12, 0x13]),
      scheme: [FieldTypes.TypeUint32, FieldTypes.TypeBytes20],
      unions: [],
      fieldNum: 1,
      newField: new Uint8Array([0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55]),
      expected: new Uint8Array([0x40, 0x00, 0x00, 0x0, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55]),
      err: false,
    },
    {
      buf: new Uint8Array([0x40, 0x00, 0x00, 0x0,0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x9, 0xa, 0xb, 0xc, 0xd, 0xe, 0xe, 0xf, 0x10, 0x11, 0x12, 0x13]),
      ...
      expected: new Uint8Array([0x40, 0x00, 0x00, 0x0,0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x9, 0xa, 0xb, 0xc, 0xd, 0xe, 0xe, 0xf, 0x10, 0x11, 0x12, 0x13]),
      err: true,
    },
    {
      buf: new Uint8Array([0x40, 0x00, 0x00, 0x0,0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x9, 0xa, 0xb, 0xc, 0xd, 0xe, 0xe, 0xf, 0x10, 0x11, 0x12, 0x13]),
  ...
  for (const tt of tests) {
    ...
    if (tt.err) {
      expect(() => {
        m.setBytes20(tt.fieldNum, tt.newField);
      }).toThrow();
    } else {
      expect(() => {
        m.setBytes20(tt.fieldNum, tt.newField);
