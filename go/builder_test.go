...
func TestBuilderUint256(t *testing.T) {
	w := InternalBuilder{}
	v := big.NewInt(65535)
	w.WriteUint256(nil, v)
	if w.size != 32 {
		t.Fatalf("instead of expected size got %v", w.size)
	}
	buf := make([]byte, w.size)
	...
	vSmallNumber := big.NewInt(63)
	w.Reset()
	w.WriteUint256(buf, vSmallNumber)
	expectedSmallNumber := []byte{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x3f}
	if !bytes.Equal(buf, expectedSmallNumber) {
		t.Fatalf("expected \"%v\" but got \"%v\"", expectedSmallNumber, buf)
	}
}
