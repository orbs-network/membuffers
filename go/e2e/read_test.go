...
var transactionBufTruncated = []byte{
	// Transaction
	// TransactionData
	0x58,0x00,0x00,0x00, // size
	0x33,0x00,0x00,0x00,
...
func TestReadTruncatedTransaction(t *testing.T) {
	transaction := types.TransactionReader(transactionBufTruncated)
	// check that the signature field is available
	sig := transaction.Signature()
	if !reflect.DeepEqual(sig, []byte{0x11,0x22,0x33,0x44,0x55,0x66}) {
		t.Fatalf("Signature: instead of expected got %v", sig)
	}
